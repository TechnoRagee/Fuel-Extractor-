"""
HPCL Outlets Database to Excel & CSV Exporter.
Reads hpcl_outlets.db and exports a multi-sheet formatted Excel file and CSV.
Features:
...
- Professional HPCL Red/Navy styling, auto-adjusted column widths, and gridlines
- Exports clean hpcl_outlets.csv
"""
...
DB_PATH = "hpcl_outlets.db"
EXCEL_PATH = "hpcl_outlets.xlsx"
CSV_PATH = "hpcl_outlets.csv"

# Styling Palette (HPCL Red/Navy Theme)
HEADER_FILL = PatternFill(start_color="8B0000", end_color="8B0000", fill_type="solid")  # Deep Crimson Red
HEADER_FONT = Font(name="Segoe UI", size=11, bold=True, color="FFFFFF")
TITLE_FONT = Font(name="Segoe UI", size=14, bold=True, color="8B0000")
...
def style_data_sheet(ws, df, sheet_title):
    ...
    # Title Block
    ws.insert_rows(1, 2)
    ws.cell(row=1, column=1, value=f"HINDUSTAN PETROLEUM (HPCL) — {sheet_title.upper()}").font = TITLE_FONT
    ws.cell(row=2, column=1, value=f"Total Stations: {len(df):,} | Exported from hpcl_outlets.db").font = SUBTITLE_FONT
    ...
    for r_idx in range(header_row + 1, header_row + 1 + len(df)):
        ...
        for c_idx in range(1, len(df.columns) + 1):
            ...
            if col_name in ["Outlet ID", "Pincode"]:
...
def build_summary_sheet(wb, df):
    ...
    # Title Block
    ws.cell(row=1, column=1, value="HINDUSTAN PETROLEUM — STATE-WISE DISTRIBUTION").font = TITLE_FONT
...
def export_db(db_path=DB_PATH, excel_path=EXCEL_PATH, csv_path=CSV_PATH):
    ...
    print("=" * 60)
    print(" EXPORTING HPCL DATABASE TO EXCEL & CSV")
    print("=" * 60)

    conn = sqlite3.connect(db_path)
    df = pd.read_sql_query("SELECT * FROM outlets ORDER BY state, city, locality, dealer_name", conn)
    conn.close()
