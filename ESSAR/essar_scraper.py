"""
Essar Oil / Nayara Energy Retail Fuel Stations Scraper.
Self-contained nationwide extraction pipeline for all former Essar Oil stations
(rebranded to Nayara Energy) across India (~6,500+ petrol pumps).

Features:
- 100% self-contained & autonomous (no external script dependencies)
- Directory-safe: works whether executed from inside ESSAR/ or from workspace root
...
- Resumable state persistence via essar_checkpoint.json
"""
...
DB_FILE = os.path.join(SCRIPT_DIR, "essar_outlets.db")
CHECKPOINT_FILE = os.path.join(SCRIPT_DIR, "essar_checkpoint.json")
...
def init_database(db_path=DB_FILE):
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS outlets (
            ...
            ro_name TEXT,
            former_brand TEXT DEFAULT 'Essar Oil',
            current_brand TEXT DEFAULT 'Nayara Energy',
            address TEXT,
            village TEXT,
            taluka TEXT,
            district TEXT,
    ...
    """)
    cur.execute("CREATE INDEX IF NOT EXISTS idx_essar_state ON outlets (state)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_essar_district ON outlets (district)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_essar_pincode ON outlets (pincode)")
    conn.commit()
    conn.close()
...
session_mgr = EssarSessionManager()
...
def query_radius(lat, lon, radius=50, delay=0.3, max_retries=3):
    """Queries Nayara/Essar API for a given coordinate."""
    if delay > 0:
        time.sleep(delay)
...
def run_scraper(workers=4, delay=0.3, radius=50, max_points=None, reset=False):
    """Runs nationwide Essar Oil / Nayara Energy scraping pipeline."""
    if reset:
    ...
    print("=" * 70)
    print(" ESSAR OIL / NAYARA ENERGY FUEL STATIONS EXTRACTOR")
    print("=" * 70)
    print("  * Notice: Essar Oil's Indian retail stations were rebranded to Nayara Energy in 2018.")
    print("  * Total Mesh Points:     " + f"{total_points:,}")
    print("  * Already Completed:     " + f"{len(completed_set):,}")
    print("  * Pending in This Run:   " + f"{len(pending_points):,}")
    print("  * Search Radius:         " + f"{radius} km")
    print("  * Existing DB Outlets:   " + f"{initial_count:,}")
    print("  * Workers (Threads):     " + f"{workers}")
    print("  * Polite Delay:          " + f"{delay}s")
    print("=" * 70)
                            ...
                            for p in pumps:
                                ...
                                cur.execute("""
                                    INSERT OR IGNORE INTO outlets (
                                        cms_code, ro_name, former_brand, current_brand,
                                        address, village, taluka, district, state, pincode,
                                        latitude, longitude, efp, petrol_price, diesel_price, created_at
                                    ) VALUES (
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrape Essar Oil / Nayara Energy Fuel Stations across India.")
